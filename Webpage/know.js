...
function onSuccess(position){
    ...
    fetch(`https://api.opencagedata.com/geocode/v1/json?q=${latitude}+${longitude}&key=c65365190efd4f69a00cffc99af335c4`)
    ...
    .then(response => response.json()).then(response =>{
        ...
        let {city_district,postcode, city} = allDetails; //getting country, postcode, country properties value from allDetails obj
        button.innerText = `${city_district} ${city}, ${postcode}`; //passing these value to the button innerText
        console.log(allDetails.city_district)
        if(allDetails.city_district== "Zone 9")
        {
            console.log("Gopalapuram")
            document.getElementById("area").value="Gopalapuram"
        }
